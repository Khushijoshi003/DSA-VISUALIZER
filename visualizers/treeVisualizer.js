...
export function renderTree(root, containerId = "#tree-visual") {
  ...
  if (!root) {
    console.warn("⚠️ Cannot render tree: root is null or undefined");
    return;
  }
...
// ✅ Animation helper to highlight nodes
...
// ✅ Main tree visualizer function
export async function visualizeTree(type = "inorder", containerId = "#tree-visual") {
  const root = sampleBinaryTree; // already a tree object
  console.log("🌳 Sample tree object:", root); // debug

  if (!root) {
    console.error("❌ visualizeTree: root is null or undefined");
    return;
  }
  ...
  switch (type) {
    ...
    case "postorder":
      ...
      break;
    default:
      console.warn(`⚠️ Unknown traversal type: "${type}"`);
  }
}
